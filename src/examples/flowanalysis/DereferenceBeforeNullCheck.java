...
/**
 * Class to demonstrate application of BD.PB.DEREF rule.
 * In order to get the violation in 'closeReader' method reported,
 * 'Do not report violation when origin cannot be show' check box 
 * in Flow Analysis options pane must be unchecked since the violation
 * has several origins.
 */
abstract class DereferenceBeforeNullCheck
{
    ...
    String readFirstLine(String sFileName)
    {
        ...
        try {
            ...
            // to obtain an alternative reader we are trying to use factory method
    ...
    {
        if (reader == null) { // VIOLATION (reported only if 'Do not report violation when origin cannot be show' check box is unchecked)
            return;
        }
        try {
            reader.close();
