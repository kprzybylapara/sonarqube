...
/**
 * <p>
 * This example shows how enforcing BD.SECURITY.VPPD: ("Encapsulate all dangerous data
 * returning methods with a validation function") helps to definitely secure an
 * application from dangerous inputs.
 * </p>
 * <p>
 ...
 * <code>HttpServletRequest</code> and propagates without being validate to
 * the SQL statement. BD.SECURITY.VPPD tell us that <code>HttpServletRequest</code> is
 * returning tainted data that is not encapsulated by a validating method.</li>
 * <li>Fix the problem reported by uncommenting the <code>PARTIAL FIX</code>
 * in the <code>authorize()</code> method.</li>
 * <li>Rerun "Security Assessment".</li>
 * </ol>
 * Note that the BD.SECURITY.TDSQL errors are no longer reported, but the
 * BD.SECURITY.VPPD errors are still reported. One could leave the code as it is as the
 * fix successfully prevents any attack on the existing code. To see how this
 * could be a problem imagine that later on somebody does the code change in
 * <code>CODE ENHANCEMENT</code> in <code>doGet()</code>. After that change
 * the code would be vulnerable to an attack again. A user would still not be
 * able to get authorized by just entering <code>"a' OR 'a' = 'a"</code> as
 * the previous fix would prevent that, but somebody could instead enter
 * <code>"hacker"</code> for username and
 * <code>"unknown'; DELETE FROM Accounts; --"</code> for password and
 * completely delete the "Accounts" table (or any other table) from the
 * database.
 * <p>
...
public class InputValidation extends HttpServlet {
    ...
    void logInvalidLogin(String username, String password) {
        try {
            ...
            String query = "INSERT INTO invalid_logins VALUES ('" + username
                    + "', '" + password + "')";
            stmt.execute(query);
